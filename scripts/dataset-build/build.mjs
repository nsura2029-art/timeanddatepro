...
function joinDatasets({ timezones, countries, cities, unRegions, holidays }) {
  ...
  // Featured cities = top N by population + every national capital.
  // Tuned to ~5,000 (per the locked spec). Override with TDP_CITY_TOP_N
  // env var (e.g. `TDP_CITY_TOP_N=8000 node build.mjs` for a bigger set).
  const TOP_N = parseInt(process.env.TDP_CITY_TOP_N || "5000", 10);
  const sorted = [...cities].sort((a, b) => b.population - a.population);
  const topByPop = sorted.slice(0, TOP_N);
  const topPopIds = new Set(topByPop.map((c) => c.geonameId));
  // Ensure every national capital is included (some small countries
  // have capitals < 100K that wouldn't make the top-N by population).
  const capitalExtras = cities.filter(
    (c) => c.isCapital && !topPopIds.has(c.geonameId)
  );
  const featuredCities = [...topByPop, ...capitalExtras].sort(
    (a, b) => b.population - a.population
  );

  log(`  ✓ ${featuredCities.length} featured cities (top ${TOP_N} by population + ${capitalExtras.length} extra capitals)`);
...
// Filter: top ${cities.length} cities by population + every national capital.
// Tuned to ~5,000 (override with TDP_CITY_TOP_N env var).
...
async function generateStats(joined) {
  ...
  log(`  Cities:       ${stats.cities} (top ${process.env.TDP_CITY_TOP_N || 5000} by pop + every national capital)`);
